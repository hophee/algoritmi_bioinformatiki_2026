...
def linear_space_alignment(s1_full, s2_full):
    res1, res2 = [], []
    stack = [('solve', s1_full, s2_full)]

    while stack:
        task = stack.pop()

        if task[0] == 'edge':
            _, s1, s2, i1, j1, i2, j2 = task
            if i2 > i1:
                res1.append(s1[i1])
            else:
                res1.append('-')
        ...
        stack.append(('solve', s1[i2:], s2[j2:]))
        stack.append(('edge', s1, s2, i1, j1, i2, j2))
        stack.append(('solve', s1[:i1], s2[:j1]))
...
with open("results_store/res_ba5l.txt", "w") as f:
    f.write(f"{score}\n{a1}\n{a2}")
